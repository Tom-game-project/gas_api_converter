...
///
use crate::{
    Class, Conv2JsTypeString, InterfaceType, Js2WitConvertErr, Method, TypeDefineLocation,
    WitTypeString, find_type_define_location, get_interface_name_from_js_type, wit_gen_func_def,
};
use crate::{json_struct::ApiService, wit::JsTypeString};
    ...
    T: IntoIterator<Item = &'a TypeRequirements>,
{
    ...
    for i in deps_uses {
        //println!("{:?}", i.blue());
        if let Some(a) = find_type_define_location(self_service_set, i) {
            rlist.push(a);
        }
    }
    rlist
...
fn get_func_def_string_vec_with_filter(
    ...
    for j in methods {
        match wit_gen_func_def(j) {
            Ok(b) => {
                rlist.push(b.0.conv2_wit_func_def());
            }
            Err(Js2WitConvertErr::NotPrimitiveType {
                wit_type_string,
                unknown_fields: unknown_interface,
            }) => {
...
fn generate_wit_methods_in_service_with_filter(
    ...
    allowed_interfaces: &[JsTypeString],
) -> Result<HashMap<JsClassName, (Vec<WitFuncDef>, HashSet<TypeRequirements>)>, Js2WitConvertErr> {
...
pub fn generate_wit_definition_with_filter(
    ...
    let ts = all_method.values().flat_map(|(_, hs)| hs.iter()).collect();

    let mut defined_interfaces = vec![];
    // サービス内のすべてのクラス
    for class in &target_service.classes {
        // クラスごとに処理して
        if let Ok(wit_interface) = generate_wit_interface(class, &all_method, &ts) {
            if allowed_interfaces.contains(&JsTypeString(
                get_interface_name_from_js_type(&class.name.clone().conv2_js_type_string())
                    .unwrap()
                    .1
                    .to_string(),
            )) {
    ...
    // サービスで必要となるもの
    let deps_uses = required_find_type_define_location(api_services, ts)
        .iter()
        .filter(|i| i.service.0 != target_service.service_name) // サービスを超えてimportするもの
        .map(|i| {
            let c = &i.class.0.clone().conv2_js_type_string();
            let (_, a) = get_interface_name_from_js_type(c).unwrap();
...
fn generate_wit_interface(
    ...
    if let Some((_wit_class_type, wit_class_name)) = get_interface_name_from_js_type(&js_class_name)
    {
        let required_type_list: Vec<TypeRequirements> = class_hashmap
...
fn generate_wit_data_type(
    ...
    if let Some((wit_class_type, wit_class_name)) = get_interface_name_from_js_type(&js_class_name)
    {
        match wit_class_type {
            InterfaceType::Class => {
                let func_defines = class_hashmap
                    .get(&js_class_name.clone().conv2_class_name())
                    .unwrap()
                    .0
                    .iter()
            ...
            InterfaceType::Enum => Ok(WitDataType::WitInterfaceEnum(WitInterfaceEnum {
                name: wit_class_name.to_string(),
