...
pub fn find_type_define_location(
    ...
    self_service_set.iter().find_map(|i| {
        i.classes.iter().find_map(|j| {
            if is_self_type(j, js_type) {
                Some(TypeDefineLocation {
...
pub fn is_self_type(self_class: &Class, type_requirements: &TypeRequirements) -> bool {
    if let Some((_, sliced)) =
        get_interface_name_from_js_type(&JsTypeString(self_class.name.clone()))
    {
...
pub fn is_in_same_service(self_service: &ApiService, js_type: &TypeRequirements) -> bool {
    self_service
        .classes
        .iter()
        .any(|a| is_self_type(a, js_type))
}
...
pub fn is_in_somewhere_service(
    ...
    self_service_set
        .iter()
        .any(|service| is_in_same_service(service, js_type))
}
