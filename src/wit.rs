...
use crate::{
    Type, TypeTrait, get_interface_name_from_js_type,
    json_struct::{ApiService, Method, Parameter},
};
...
fn wit_convert_arg_type_pair(parameter: &Parameter) -> Result<WitTypeString, Js2WitConvertErr> {
    let a = convert_wit_type_string(&parameter.param_type);
    if let Ok(b) = a {
        Ok(WitTypeString(format!("{}: {}", parameter.name, b.0)))
    ...
    }) = a
    {
        Err(Js2WitConvertErr::NotPrimitiveType {
            wit_type_string: WitTypeString(format!("{}: {}", parameter.name, wit_type_string.0)),
            unknown_fields,
        })
    } else {
        Err(Js2WitConvertErr::ParameterStringErr(format!(
            "{parameter:?}"
        )))
    }
}
    ...
    T: TypeTrait,
{
    if arg.get_name().ends_with("[]") {
        ...
        let wit_type = convert_wit_type_string(&Type {
            name: b.to_string(),
            url: arg.get_url().clone(),
        });
...
{
    // primitive type
    if arg.get_url().is_none()
    // 説明のurlが無い -> プリミティブな型を疑う
    {
    ...
    if is_ascii_alnum_or_underscore(&arg.get_name()) {
        return Err(Js2WitConvertErr::NotPrimitiveType {
            wit_type_string: WitTypeString(arg.get_name().to_case(Case::Kebab)),
...
/// witの関数宣言の引数部分を作成する
pub fn wit_parameters_string(
    parameter_list: &Vec<Parameter>,
) -> Result<WitTypeString, Js2WitConvertErr> {
    let mut rlist: Vec<String> = vec![];
    let mut unknown_fields_gather = vec![];

    for i in parameter_list {
        let arg_type = wit_convert_arg_type_pair(i);
...
// jsonのmethodnameの記述が`(...)`で終了することを保証してもらう必要がある
{
    ...
    let wit_return = convert_wit_type_string(&method.return_type);
...
",
    ...
    let func_name = extract_method_name(&method.name)
        .expect("Error: not end with `()`")
        .to_case(Case::Kebab);
    let func_type = if wit_return.0 == "void" {
        format!("func ({})", wit_parameters.0,)
    } else {
...
///
pub fn wit_gen_interface_name(
    js_type_name: &JsTypeString,
) -> Result<WitTypeString, Js2WitConvertErr> {
    if let Some((_, sliced)) = get_interface_name_from_js_type(js_type_name) {
        Ok(WitTypeString(sliced.to_case(Case::Kebab)))
    } else {
        Err(Js2WitConvertErr::WrongFormatErr)
    }
}

pub fn wit_gen_package_name(
    prefix: &str,
    target_service: &ApiService,
    wit_version: Option<&WitTypeString>,
) -> WitTypeString {
    WitTypeString(format!(
        "package {}:{}{}",
        prefix,
...
pub fn wit_gen_service_use(
    prefix: &str,
    target_service: &JsTypeString,
    target_interface: &JsTypeString,
    wit_version: Option<&WitTypeString>,
) -> Result<WitTypeString, Js2WitConvertErr> {
    let a = wit_gen_interface_name(target_interface)?;

    Ok(WitTypeString(format!(
        "use {}:{}/{}{}",
        prefix,
        target_service.0,
        a.0,
...
pub fn wit_gen_interface_use(
    target_resource: &JsTypeString,
) -> Result<WitTypeString, Js2WitConvertErr> {
    let a = wit_gen_interface_name(target_resource)?;

    Ok(WitTypeString(format!("use {}.{{{}}}", a.0, a.0)))
}
